...
#define __MAX_VECTOR_SIZE       (1u << 30)
#define __MAX_VECTORS_COUNT     (1u << 20)
#define __MAX_STACK_HEIGHT      (1u << 20)
#define __MAX_ARRAYS_COUNT      (1u << 5)
#define __MAX_ALLOC_SIZE        (1u << (__MAX_ARRAYS_COUNT - 1))
#define __MAX_VECTOR_DIMENSION  (1u << 5)
...
#else
# define vt_delete(vt)            (__vt_delete(vt, __true))  // in multi-threading.
#endif
...
typedef struct {
  ...
  size_t  arrays_counts[__MAX_ARRAYS_COUNT];
  size_t  arrays_count;
  size_t  type_size;
  size_t  count;
  size_t  __dimension;
...
int     __vt_insert(vector self, size_t idx, void *item);
int     __vt_input(vector self, size_t idx, ...);
