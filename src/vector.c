...
static vector   __vt_vectors[MAX_VECTOR_COUNT];
...
static size_t   __vt_maximum_bit(size_t n)
{
...
static void __vt_pre_incre(vector self, size_t count)
{
...
static void __vt_clean(void)
{
    while (__vt_vectors_count)
        __vt_delete(__vt_vectors[--__vt_vectors_count], _false);
}
...
static vector   __vt_new_recurse(
    size_t type_size, size_t dimension, size_t *counts)
{
    ...
    if (dimension)
        for (size_t i = 0; i < vt->__count; ++i)
            vt_at(vt, i, vector) = __vt_new_recurse(type_size, dimension, counts);
...
vector  __vt_new(size_t type_size, size_t dimension, ...)
{
    ...
    {
        __vt_is_registered_atexit = _true;
        atexit(__vt_clean);
    }
#endif
    if (!dimension || dimension > MAX_VECTOR_DIMENSION)
        return _nullptr;

    va_list ap;
    va_start(ap, dimension);
    size_t counts[MAX_VECTOR_DIMENSION];
...
void    __vt_reverse(vector self)
{
    ...
    while (l < r)
        __vt_memswap(
            self->__array + l++ * self->__type_size,
